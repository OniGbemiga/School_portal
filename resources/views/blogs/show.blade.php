@extends('layouts.body')

@section('content')

        <!--Banner Wrap Start-->
        <div class="kf_inr_banner">
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                    	<!--KF INR BANNER DES Wrap Start-->
                        <div class="kf_inr_ban_des">
                        	<div class="inr_banner_heading">
								<h3>Blog Detail</h3>
                        	</div>

                            <div class="kf_inr_breadcrumb">
								<ul>
									<li><a href="/index">Home</a></li>
									<li><a href="/blogs">Blog</a></li>
									<li><a href="#">Blog Detail</a></li>
                                    @if(!auth::guest())
                                        @if(Auth::user()->id == $blogs->user_id)
                                            <li><a href="/blogs/{{$blogs->id}}/edit">Edit Blog</a></li>
                                        @endif
                                    @endif
								</ul>
							</div>
                        </div>
                        <!--KF INR BANNER DES Wrap End-->
                    </div>
                </div>
            </div>
        </div>

        <!--Banner Wrap End-->

    	<!--Content Wrap Start-->
    	<div class="kf_content_wrap">
    		<section>
    			<div class="container">
    				<div class="row">
    					<div class="col-md-8">

    						<!--KF_BLOG DETAIL_WRAP START-->
    						<div class="kf_blog_detail_wrap">

    							<!-- BLOG DETAIL THUMBNAIL START-->
    							<div class="blog_detail_thumbnail">
	    							<figure>
	    								<img src="/storage/blog_image/{{$blogs->blog_image}}" alt="" style="width: 750px; height:500px"/>
	    								<figcaption><a href="#comments">education</a></figcaption>
	    							</figure>
	    						</div>
	    						<!-- BLOG DETAIL THUMBNAIL END-->

    							<!--KF_BLOG DETAIL_DES START-->
    							<div class="kf_blog_detail_des">
	    							<div class="blog-detl_heading" style="align-content: inherit">
                                        <div style="float: left">
	    								    <h5>{{$blogs->title}}</h5>
	    							    </div>
                                        @if(!auth::guest())
                                            @if(auth()->user()->id == $blogs->user_id)
                                                <div style="float: right">
                                                    <button class="btn btn-outline-primary"><a href="/blogs/{{$blogs->id}}/edit">Edit</a></button>
                                                </div>
                                            @endif
                                        @endif
                                    </div>

    								<ul class="blog_detail_meta">
    									<li><i class="fa fa-calendar"></i><a href="#">{{\Carbon\Carbon::parse($blogs->created_at)->diffForHumans()}}</a></li>
    									<li><i class="fa fa-heart-o"></i><a href="#">69 Likes</a></li>
    									<li><i class="fa fa-comments-o"></i><a href="#comments">{{count($blogs->comments)+count($blogs->replies)}} Comments</a></li>
    								</ul>

	    							<p>{{$blogs->main_body}}</p>


	    							<blockquote>
										<p>{{$blogs->quotes}}</p>
									</blockquote>


	    							<p class="margin-bottom">Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Mauris in erat justo. Nullam ac urna eu felis dapibus condimentum sit amet a augue. Sed non neque elit. Sed ut imperdiet nisi. Proin condimentum fermentum nunc. </p>

	    							<!--KF_BLOG THUMB wRAP START-->
	    							<div class="blog_thumb_wrap">
		    							<div class="row">
		    								<div class="col-md-4 col-sm-4 ">
		    									<figure>
		    										<img src="{{asset('extra-images/blog_detail1.jpg')}}" alt=""/>
		    										<figcaption><a href="#"><i class="fa fa-search"></i></a></figcaption>
		    									</figure>
		    								</div>
		    								<div class="col-md-4 col-sm-4">
		    									<figure>
		    										<img src="{{asset('extra-images/blog_detail2.jpg')}}" alt=""/>
		    										<figcaption><a href="#"><i class="fa fa-search"></i></a></figcaption>
		    									</figure>
		    								</div>
		    								<div class="col-md-4 col-sm-4">
		    									<figure>
		    										<img src="{{asset('extra-images/blog_detail3.jpg')}}" alt=""/>
		    										<figcaption><a href="#"><i class="fa fa-search"></i></a></figcaption>
		    									</figure>
		    								</div>
		    							</div>
		    						</div>
		    						<!--KF_BLOG THUMB wRAP END-->

	    							<p class="margin-bottom">Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos. Mauris in erat justo. Nullam ac urna eu felis dapibus condimentum sit amet a augue. </p>
								</div>
                                @if(!Auth::guest())
                                    @if(Auth::user()->id == $blogs->user_id)
                                        <form action="/blogs/{{$blogs->id}}", method="POST">
                                            @method('DELETE')
                                            @csrf
                                            <button class="alert alert-danger">Delete Post</button>
                                        </form>
                                    @endif
                                @endif
	    						<!--KF_BLOG DETAIL_DES END-->

	    						<!--KF_BLOG ABOUT AUTHOR START--style="width: 125px; height:125px"-->
	    						<div class="about_autor">
	    							<figure><img src="/storage/user_image/{{$blogs->user->user_image}}" alt=""></figure>
	    							<div class="about-autor_des">
	    								<h6><a href="#">{{$blogs->user->name}}</a></h6>
	    								<p>{{$blogs->user->user_profile}}</p>
	    								<ul class="autor_social">
	    									<li><a href="#"><i class="fa fa-facebook"></i></a></li>
	    									<li><a href="#"><i class="fa fa-twitter"></i></a></li>
	    									<li><a href="#"><i class="fa fa-google-plus"></i></a></li>
	    									<li><a href="#"><i class="fa fa-linkedin"></i></a></li>
	    								</ul>
	    							</div>
	    						</div>
	    						<!--KF_BLOG ABOUT AUTHOR END-->

	    						<!--SECTION COMMENT START-->
	    						<div class="section-comment" id="comments">
	    							<div class="blog-detl_heading">
	    								<h5>{{count($blogs->comments) + count($blogs->replies)}} Comments</h5>
	    							</div>

	    							<!-- COMMENT LIST START-->
    								<ul class="coment_list">
    									<li>
											<!-- COMMENT WRAP START-->
											@foreach ($blogs->comments as $comment)
												<div class="comment_wrap">
													<figure><img src="/storage/user_image/{{$comment->user->user_image}}" alt="" style="width: 108px;height:93px"/></figure>
													<div class="comment_des">
														<div class="comment_des_hed">
															<cite><a href="#">{{$comment->user->name}}</a><span>{{$comment->created_at->diffForHumans()}}</span></cite>
															{{-- <a class="comment_reply" href="#"><i class="fa fa-mail-reply"></i>Reply</a> --}}
															<button class="comment_reply" data-commentbox="{{$comment->id}}"><i class="fa fa-mail-reply"></i>Reply</button>
														</div>

														<p>{{$comment->comment_body}}</p>
														<form action="{{route('replying')}}" method="POST">
															@csrf
															<div class="replybox" id="{{$comment->id}}" style="display: none">
																<textarea cols="60" style="border: 2px solid black" name="reply_body"></textarea>
																<input type="hidden" value="{{$blogs->id}}" name="blog_id">
																<input type="hidden" value="{{$comment->id}}" name="comment_id">
																<button class="sendbutton">Send</button>
																<button class="cancelbutton">Cancel</button>
															</div>
														</form>

														@if ($comment->replies)
															@foreach ($comment->replies as $reply)
																<ul class="secnd_coment_list">
																	<li>
																		<!-- COMMENT WRAP START-->
																		<div class="comment_wrap">
																			<figure><img src="/storage/user_image/{{$reply->user->user_image}}" alt="" style="width: 108px;height:93px"/></figure>
																			<div class="comment_des">
																				<div class="comment_des_hed">
																					<cite><a href="#">{{$reply->user->name}}</a><span>{{$reply->created_at->diffForHumans()}}</span></cite>
																					{{-- <button class="comment_reply" data-commentbox="{{$reply->id}}"><i class="fa fa-mail-reply"></i>Reply</button> --}}
																					{{-- <button class="replybutton" data-commentbox="panel1">Reply</button> --}}
																				</div>
																				<p>{{$reply->reply_body}}</p>
																				{{-- <form action="{{route('replying')}}" method="POST">
																					@csrf
																					<div class="replybox" id="{{$reply->id}}" style="display: none">
																						<textarea cols="60" style="border: 2px solid black" name="reply_body"></textarea>
																						<input type="hidden" value="{{$blogs->id}}" name="blog_id">
																						<input type="hidden" value="{{$comment->id}}" name="comment_id">
																						<input type="hidden" value="{{Auth::user()->id}}" name="user_id">
																						<button class="cancelbutton">Send</button>
																					</div>
																				</form> --}}
																			</div>
																		</div>
																		<!-- COMMENT WRAP END-->
																	</li>
																</ul>
															@endforeach
														@endif
													</div>
												</div>
											@endforeach
											<!-- COMMENT WRAP END-->
										</li>
    								</ul>
    								<!-- COMMENT LIST END-->
	    						</div>
	    						<!--SECTION COMMENT END-->

	    						<!-- BLOG PG FORM START-->
	    						<div class="blog_pg_form">
	    							<div class="blog-detl_heading">
	    								<h5>Leave A Message</h5>
	    							</div>
	    							<form action="{{route('storing')}}" method="POST">
										@csrf
	    								<div class="row">
	    									<div class="col-md-6">
	    										<blockquote>Please Sign In To Comment</blockquote>
											</div>
											<div class="col-md-6">
	    										<input type="hidden" value="{{$blogs->id}}" name="blog_id">
	    									</div>
	    									<div class="col-md-12">
	    										<textarea placeholder="Message" name="comment_body"></textarea>
	    									</div>
	    								</div>
										<button>Send Comment</button>
	    							</form>
	    						</div>
	    						<!-- BLOG PG FORM END-->

    						</div>
    						<!--KF_BLOG DETAIL_WRAP END-->
    					</div>

    					<!--KF_EDU_SIDEBAR_WRAP START-->
    					<div class="col-md-4">
    						<div class="kf-sidebar">

    							<!--KF_SIDEBAR_SEARCH_WRAP START-->
    							<div class="widget widget-search">
                                	<h2>Search Course</h2>
    								<form>
    									<input type="search" placeholder="Keyword...">
    								</form>
    							</div>
    							<!--KF_SIDEBAR_SEARCH_WRAP END-->

    							<!--KF_SIDEBAR_ARCHIVE_WRAP START-->
    							<div class="widget widget-archive ">
    								<h2>Archives</h2>
    								<ul class="sidebar_archive_des">
    									<li>
    										<a href="#"><i class="fa fa-angle-right"></i>January</a>
    									</li>
    									<li>
    										<a href="#"><i class="fa fa-angle-right"></i>February</a>
    									</li>
    									<li>
    										<a href="#"><i class="fa fa-angle-right"></i>March</a>
    									</li>
    									<li>
    										<a href="#"><i class="fa fa-angle-right"></i>April</a>
    									</li>
    									<li>
    										<a href="#"><i class="fa fa-angle-right"></i>May</a>
    									</li>
    								</ul>
    							</div>
    							<!--KF_SIDEBAR_ARCHIVE_WRAP END-->


    							<!--KF SIDEBAR RECENT POST WRAP START-->
    							{{-- <div class="widget widget-recent-posts">
									<h2>Recent Posts</h2>
									@foreach ($blogs as $blog)
										<ul class="sidebar_rpost_des">
											<!--LIST ITEM START-->
											<li>
												<figure>
													<img src="/storage/blog_image/{{$blogs->blog_image}}" alt="">
													<figcaption><a href="blogs/{{$blogs->id}}"><i class="fa fa-search-plus"></i></a></figcaption>
												</figure>
												<div class="kode-text">
													<h6><a href="blogs/{{$blogs->id}}">{{$blogs->title}}</a></h6>
													<span><i class="fa fa-clock-o"></i>{{$blogs->created_at->diffForHumans()}}</span>
												</div>
											</li>
											<!--LIST ITEM START-->
										</ul>
									@endforeach
								</div> --}}
    							<!--KF SIDEBAR RECENT POST WRAP END-->

    							<!--KF EDU SIDEBAR COURSES CATEGORieS WRAP START-->
	    						<div class="widget widget-categories">
	    							<h2>categories</h2>
									<ul>
										<li><a href="#"><i class="fa fa-caret-right"></i>Business &amp; Economics</a></li>
										<li><a href="#"><i class="fa fa-caret-right"></i>Politics &amp; History</a></li>
										<li><a href="#"><i class="fa fa-caret-right"></i>Medical Sciences &amp; Health</a></li>
										<li><a href="#"><i class="fa fa-caret-right"></i>Fine Arts</a></li>
										<li><a href="#"><i class="fa fa-caret-right"></i>Tourism &amp; Culture</a></li>
										<li><a href="#"><i class="fa fa-caret-right"></i>Sports</a></li>
									</ul>
	    						</div>
	    						<!--KF EDU SIDEBAR COURSES CATEGORieS WRAP END-->

	    						<!--KF SIDE BAR COURSES LIST WRAP START WRAP START-->
	    						<div class="widget widget-courses-list">
	    							<h2>Latest Courses</h2>
	    							<ul>
	    								<li>
                                        	<figure>
                                            	<img src="{{asset('extra-images/courseslist1.jpg')}}" alt=""/>
	    										<a href="#">View Detail</a>
	    									</figure>
	    								</li>

	    								<li>
                                        	<figure>
                                            	<img src="{{asset('extra-images/courseslist2.jpg')}}" alt=""/>
	    										<a href="#">View Detail</a>
	    									</figure>
	    								</li>

	    								<li>
                                        	<figure>
                                            	<img src="{{asset('extra-images/courseslist3.jpg')}}" alt=""/>
	    										<a href="#">View Detail</a>
	    									</figure>
	    								</li>

	    								<li>
                                        	<figure>
                                            	<img src="{{asset('extra-images/courseslist4.jpg')}}" alt=""/>
	    										<a href="#">View Detail</a>
	    									</figure>
	    								</li>

	    								<li>
                                        	<figure>
                                            	<img src="{{asset('extra-images/courseslist5.jpg')}}" alt=""/>
	    										<a href="#">View Detail</a>
	    									</figure>
	    								</li>

	    								<li>
                                        	<figure>
                                            	<img src="{{asset('extra-images/courseslist6.jpg')}}" alt=""/>
	    										<a href="#">View Detail</a>
	    									</figure>
	    								</li>
	    							</ul>
	    						</div>
	    						<!--KF SIDE BAR COURSES LIST WRAP START WRAP END-->

	    						<!--KF SIDE BAR TAG CLOUD WRAP START-->
	    						<div class="widget widget-tag-cloud">
	    							<h2>Tags Cloud</h2>
	    							<ul>
	    								<li><a href="#">Science</a></li>
	    								<li><a href="#">Development</a></li>
	    								<li><a href="#">Fine Arts</a></li>
	    								<li><a href="#">Research</a></li>
	    								<li><a href="#">Admissions</a></li>
	    								<li><a href="#">PHD</a></li>
	    								<li><a href="#">History &amp; Politics</a></li>
	    								<li><a href="#">Sports</a></li>
	    							</ul>

	    						</div>
	    						<!--KF SIDE BAR TAG CLOUD WRAP END-->

    						</div>
    					</div>
						<!--KF EDU SIDEBAR WRAP END-->

    				</div>
    			</div>
    		</section>
        </div>

@endsection

